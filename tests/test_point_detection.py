...
def _make_observation(
    *,
    ...
    from sva.models import WindowEvents

    # v2.0 schema: score signal moved from FormationObservation to WindowEvents.
    # Map the legacy "two_hands_up" string to the structured arms_raised flag.
    arms_raised_observed = score_signal == "two_hands_up"
    arms_raised_conf = score_signal_confidence if arms_raised_observed else 0.0
    ...
    return Observation(
        ...
        events=WindowEvents(
            arms_raised_score_signal_observed=arms_raised_observed,
            arms_raised_count=arms_raised_count,
            arms_raised_score_signal_confidence=arms_raised_conf,
        ),
...
def test_detect_points_from_observations_finds_two_points_separated_by_between_phase():
    observations = [
        ...
        _make_observation(start_ms=0, end_ms=1000, phase="pre_pull", phase_confidence=0.6, pull_formation_visible=True),
        _make_observation(start_ms=1000, end_ms=2000, phase="live_play", phase_confidence=0.8),
        _make_observation(start_ms=2000, end_ms=3000, phase="post_score_celebration", phase_confidence=0.6),
        # Between points
